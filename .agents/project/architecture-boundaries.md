...
4. The installed CLI must not require network access for `init`, `check`, `sync`, `status`, or `preview`. `upgrade` may perform an opt-in best-effort version probe that recommends an update command; the probe must fail closed (silent skip) so `upgrade` still completes offline.
...
4. A change that makes `init`, `check`, `sync`, `status`, or `preview` require network access after installation, or a change that makes the `upgrade` version probe required (rather than best-effort) or expands network access to additional commands.
