...
- npm trusted publishing must be configured on npmjs.com for
  `@yysjjd/agent-feed` with GitHub user `fqmyysjjd`, repository `agent-feed`,
  workflow filename `publish.yml`, and environment `npm`. If npm returns `404`
  during `npm publish`, the package scope or trusted publisher configuration
  does not allow this workflow to publish that package.
- Release publishing is intentionally rerunnable after partial failure: PyPI
  uses `skip-existing`, npm skips an already-published package version, and the
  Homebrew update resolves the existing PyPI sdist for the release tag.
- Homebrew resource updates are generated by `scripts/update-homebrew-formula.py`
  from pip's dependency report and PyPI sdists. Do not reintroduce
  `brew update-python-resources` into the release workflow unless its
  newly-uploaded package time window is handled explicitly.
